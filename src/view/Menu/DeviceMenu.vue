  ...
  <div>
    <el-row type="flex" align="middle" justify="start">
      ...
      <el-col class="icon-action" v-if="selectable">
        <el-button icon="el-icon-delete" @click="onDelete" size="medium" circle></el-button>
      </el-col>

      <el-col class="icon-action" v-if="!selectable">
        <el-button icon="el-icon-plus" @click="onUpload" size="medium" circle></el-button>
      </el-col>

      <el-col class="icon-action">
        <el-button icon="el-icon-refresh-right" @click="onRefresh" size="medium" circle></el-button>
      </el-col>
    </el-row>
    <app-upload-dialog @onUploadDialogDismiss="onUploadClose" :visible="showUpload"/>
  </div>
</template>

<script>
import AppUploadDialog from "@/view/AppPage/AddAppDialog";
...
export default {
  ...
  methods: {
    ...
    onRefresh(e) {
      this.onClicked(e);
      this.$EventBus.$emit('fetchApp');
    },
    onDelete(e) {
      this.onClicked(e);
      this.$EventBus.$emit('deleteApps');
    },
    onClicked(e) {
      let target = e.target;
    ...
    onDialogClose() {
      //this.$index.commit('setMenuVisible', false);
    },
